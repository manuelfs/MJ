...
// 2D histograms 
//
void Make2DPlots(int version,TString HistName, int NMergeBins=1, bool doRatio=false) 
{ 
  TFile* HistFile = TFile::Open(Form("Out/v%i/HistFiles/Hist_v%i.root",version,version));
...
//        h2_DY[i]          = (TH2F*)HistFile->Get(Form("h2_DY_%s_%ifatjet", HistName.Data(), i)); 
    ...
    for(int i=2; i<7; i++) 
    {
        ...
        c_TT_sl->Print( Form("Out/v%i/Figures/2D_TT_sl_%s%s_v%i.pdf",version, 
			     HistName.Data(), DoLog?"_log":"",version) ); 
        c_TT_ll->Print( Form("Out/v%i/Figures/2D_TT_ll_%s%s_v%i.pdf",version, 
			     HistName.Data(), DoLog?"_log":"",version) ); 
    }
    ...
    { 
        for(int i=2; i<7; i++) 
        {
            ...
            c_Ratio->Print( Form("Out/v%i/Figures/2D_Ratio_%s%s_v%i.pdf",version,
				 HistName.Data(), DoLog?"_log":"",version) ); 
        } 
    }
