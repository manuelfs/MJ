...
void PrintTableOneLine(TString Process, TH1F* h1[7], int lepflav=0, bool doLatex=true, bool PrintFile=true)
{
    ...
    if(PrintFile) 
    {
      if(lepflav!=11 && lepflav!=13 && lepflav!=15) //not actually tau, 15 means dilepton
        {
            Double_t error[7];
            for(int i=2; i<7; i++) h1[i]->IntegralAndError(1,10000,error[i]);
            fout << Process << " & " 
                 << Form("$%.2f \\pm %.2f$",h1[2]->Integral(1,2),error[2]) << "\t&" 
                 << Form("$%.2f \\pm %.2f$",h1[3]->Integral(1,2),error[3]) << "\t&" 
                 << Form("$%.2f \\pm %.2f$",h1[4]->Integral(1,2),error[4]) << "\t&" 
                 << Form("$%.2f \\pm %.2f$",h1[5]->Integral(1,2),error[5]) << "\t&" 
                 << Form("$%.2f \\pm %.2f$",h1[6]->Integral(1,2),error[6]) << "\t\\\\" 
                 << endl; 
        }
        else 
        {   
    ...
    else 
    {
        ...
        if(doLatex)
        {
            if(lepflav!=11 && lepflav!=13 && lepflav!=15)
            {
                Double_t error[7];
                for(int i=2; i<7; i++) h1[i]->IntegralAndError(1,10000,error[i]);
                cout << Process << " & " 
        ...
        {   
            if(lepflav!=11 && lepflav!=13 && lepflav!=15)
            {
                ...
                cout << "|" << 
                    setw(20) << Process << " |"  <<
                    setw(20) << Form("%.2f +/- %.2f", h1[2]->Integral(1,2), error[2]) << " |" <<
                    setw(20) << Form("%.2f +/- %.2f", h1[3]->Integral(1,2), error[3]) << " |" <<
                    setw(20) << Form("%.2f +/- %.2f", h1[4]->Integral(1,2), error[4]) << " |" <<
                    setw(20) << Form("%.2f +/- %.2f", h1[5]->Integral(1,2), error[5]) << " |" <<
                    setw(20) << Form("%.2f +/- %.2f", h1[6]->Integral(1,2), error[6]) << " |" << endl; 
            }
            else 
            {   
                int bin = (lepflav-9)/2;
...
void MakeTables_all(int version, int lepflav=0, char* Region="",int nMJ=0,int nMET=0,int nMT=0,int nb=0,int nsj=0,int nHT=0, bool doLatex=false)
{ 
    ...
    for(int i=2; i<7; i++)
    {
      //"h1_%s_yields_%ifatjet_%iMJ_%iMET_%iMT_%inb_%insj", ChainName.Data(), i,nMJ,nMET,nMT,nb,nsj h1_DATA[i]      = (TH1F*)HistFile->Get(Form("h1_DATA_%s_%s_%ifatjet_%iMJ_%iMET_%iMT_%inb_%insj_%inHT", Region,HistName.Data(), i,nMJ,nMET,nMT,nb,nsj,nHT));
      h1_T[i]         = (TH1F*)HistFile->Get(Form("h1_T_%s_%s_%ifatjet_%iMJ_%iMET_%iMT_%inb_%insj_%inHT", Region,HistName.Data(), i,nMJ,nMET,nMT,nb,nsj,nHT));
      h1_TT_sl[i]     = (TH1F*)HistFile->Get(Form("h1_TT_sl_%s_%s_%ifatjet_%iMJ_%iMET_%iMT_%inb_%insj_%inHT", Region,HistName.Data(), i,nMJ,nMET,nMT,nb,nsj,nHT));
      h1_TT_ll[i]     = (TH1F*)HistFile->Get(Form("h1_TT_ll_%s_%s_%ifatjet_%iMJ_%iMET_%iMT_%inb_%insj_%inHT", Region,HistName.Data(), i,nMJ,nMET,nMT,nb,nsj,nHT));
      h1_WJets[i]     = (TH1F*)HistFile->Get(Form("h1_WJets_%s_%s_%ifatjet_%iMJ_%iMET_%iMT_%inb_%insj_%inHT", Region,HistName.Data(), i,nMJ,nMET,nMT,nb,nsj,nHT));
      h1_DY[i]        = (TH1F*)HistFile->Get(Form("h1_DY_%s_%s_%ifatjet_%iMJ_%iMET_%iMT_%inb_%insj_%inHT", Region,HistName.Data(), i,nMJ,nMET,nMT,nb,nsj,nHT));
      h1_f1500_100[i] = (TH1F*)HistFile->Get(Form("h1_T1tttt_f1500_100_%s_%s_%ifatjet_%iMJ_%iMET_%iMT_%inb_%insj_%inHT", Region,HistName.Data(), i,nMJ,nMET,nMT,nb,nsj,nHT));
      h1_f1200_800[i] = (TH1F*)HistFile->Get(Form("h1_T1tttt_f1200_800_%s_%s_%ifatjet_%iMJ_%iMET_%iMT_%inb_%insj_%inHT", Region,HistName.Data(), i,nMJ,nMET,nMT,nb,nsj,nHT));

      //if(!h1_DATA[i]) cout<<"ERROR MakeTables_all Histfile Get Data"<<endl; 
	if(!h1_T[i]) cout<<"ERROR MakeTables_all Histfile Get T"<<endl;    
	if(!h1_TT_sl[i]) cout<<"ERROR MakeTables_all Histfile Get TT sl"<<endl;  
	if(!h1_TT_ll[i]) cout<<"ERROR MakeTables_all Histfile Get TT ll"<<endl;
	
    	if(!h1_WJets[i]) cout<<"ERROR MakeTables_all Histfile Get Wjets"<<endl;     
	if(!h1_DY[i]) cout<<"ERROR MakeTables_all Histfile Get DY"<<endl;
	if(!h1_f1500_100[i]) cout<<"ERROR MakeTables_all Histfile Get noncomp"<<endl;
	if(!h1_f1200_800[i]) cout<<"ERROR MakeTables_all Histfile Get compr"<<endl;
